...
public class DatabaseManager extends SQLiteOpenHelper {
    private static final String DATABASE_NAME = "CandyDB2";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String sqlUser = "create table User (email text, password text, fname text, lname text)";
        db.execSQL(sqlUser);
    }
    ...
    public void insertUser(User user){
        SQLiteDatabase db = this.getWritableDatabase();
        String sqlInsert = "insert into User values ('" + user.getEmail() + "', '" +
            user.getPass() + "', '" + user.getFname() + "', '" + user.getLname() + "' )";
        db.execSQL(sqlInsert);
        db.close();
    }
